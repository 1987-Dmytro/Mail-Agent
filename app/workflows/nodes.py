...
async def send_telegram(
    ...
    try:
        ...
        async with db_factory() as db:
            ...
            # NOTE: All emails are sent to Telegram immediately, is_priority affects display
            is_priority = state.get("priority_score", 0) >= 70
            state["is_priority"] = is_priority

            # SEND ALL EMAILS TO TELEGRAM (was: only priority emails)
            if True:  # Changed from "if is_priority:" to send all emails
                # Send ALL emails immediately to Telegram
                logger.info("email_sending_to_telegram", email_id=state["email_id"], priority_score=state.get("priority_score"), is_priority=is_priority)
