...
describe('GmailConnect Component - Unit Tests', () => {
   ...
   * - State token is extracted from auth_url and stored in sessionStorage
  ...
  it('test_oauth_initiation_constructs_url', async () => {
    ...
    // State is extracted from backend-provided auth_url (not generated by frontend)
    // The mocked auth_url contains state='test-state' (see beforeEach setup)
    ...
    // Verify state token was extracted from auth_url and stored in sessionStorage
    expect(sessionStorage.getItem('oauth_state')).toBe(expectedState);

    // Verify window.location.href was set to the complete auth_url with state parameter
    await waitFor(() => {
      expect(window.location.href).toContain('accounts.google.com');
      expect(window.location.href).toContain(`state=${expectedState}`);
    });
